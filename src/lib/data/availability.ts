 ...
 */
export function getProviderAvailability(providerId: string): Promise<AvailabilityRule[]> {
 ...
 */
export function getProviderBlackoutDates(providerId: string): Promise<{ start_date: string; end_date: string }[]> {
  const today = new Date().toISOString().split('T')[0]

  return unstable_cache(
    async (): Promise<{ start_date: string; end_date: string }[]> => {
      const supabase = createAdminClient()
    ...
    ['provider-blackouts', providerId, today],
    { revalidate: CACHE_TIMES.availability }
  )()
}
